...
class RestaurantDaoTest : DaoTest() {
    ...
    @Test
    fun selectByNameShouldBeSuccess() = runBlocking {
        restaurantDao.insert(restaurantEntity)

        val restaurantId = restaurantDao.selectIdBy(restaurantEntity.name)

        assertThat(restaurantId).isNotNull()
        assertThat(restaurantId).isEqualTo(restaurantEntity.id)
    }
